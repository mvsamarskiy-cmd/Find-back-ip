...
def test_claimability_requires_explicit_positive_signal():
    verdict = fuse_evidence(
        "instagram",
        "example",
        [Evidence(platform="instagram", handle="example", source="public_web", method="public_profile", signal="absent", confidence=0.8).to_dict()],
    )
...
def test_explicit_claimable_signal_becomes_verified_available():
    verdict = fuse_evidence(
        ...
        "example.com",
        [
            Evidence(platform="com", handle="example.com", source="verisign_rdap", method="rdap_exact_domain", signal="absent", confidence=0.9).to_dict(),
            Evidence(platform="com", handle="example.com", source="namecom_core_api", method="registrar_check_availability", signal="claimable", confidence=0.99).to_dict(),
        ],
    )
    assert verdict.verdict == "available_verified"
...
def test_exists_evidence_blocks_absence_only_evidence():
    verdict = fuse_evidence(
        ...
        "example",
        [
            Evidence(platform="telegram", handle="example", source="search_engine", method="exact_profile_search", signal="absent", confidence=0.6).to_dict(),
            Evidence(platform="telegram", handle="example", source="public_web", method="public_profile", signal="exists", confidence=0.85).to_dict(),
        ],
    )
...
def test_claimable_plus_exists_is_never_verified_available():
    verdict = fuse_evidence(
        ...
        [
            Evidence(platform="com", handle="example.com", source="namecom_core_api", method="registrar_check_availability", signal="claimable", confidence=0.99).to_dict(),
            Evidence(platform="com", handle="example.com", source="verisign_rdap", method="rdap_exact_domain", signal="exists", confidence=0.98).to_dict(),
        ],
    )
...
def test_invalid_wins_over_claimable_signal():
    verdict = fuse_evidence(
        ...
        "bad handle",
        [
            Evidence(platform="x", handle="bad handle", source="x_api", method="lookup", signal="invalid", confidence=0.99).to_dict(),
            Evidence(platform="x", handle="bad handle", source="public_web", method="fallback", signal="claimable", confidence=0.5).to_dict(),
        ],
    )
...
def test_stronger_unresolved_source_blocks_weaker_absence_marketing():
    verdict = fuse_evidence(
        ...
        "example",
        [
            Evidence(platform="facebook", handle="example", source="whatsmyname", method="community_fingerprint", signal="absent", confidence=0.68).to_dict(),
            Evidence(platform="facebook", handle="example", source="public_web", method="public_profile", signal="unknown", confidence=0.0).to_dict(),
        ],
    )
    assert verdict.verdict == "unknown"
...
def test_collector_preserves_original_and_enriched_evidence():
    rows = collect_platform_evidence(
        "x",
        "example",
        legacy_row={"status": "not_found", "source": "public_web", "method": "public_profile", "confidence": 0.65, "url": "https://x.com/example"},
        enriched_row={"status": "taken", "source": "socialscan", "method": "registration_probe", "confidence": 0.9, "url": "https://x.com/example"},
    )
    assert len(rows) == 2
...
def test_collector_deduplicates_unchanged_compatibility_rows():
    row = {"status": "taken", "source": "public_web", "method": "public_profile", "confidence": 0.85, "url": "https://example.test/user"}
...
def test_collector_fuses_conflict_instead_of_last_writer_wins():
    verdicts = collect_verification_verdicts(
        "example",
        {"x": {"status": "claimable", "source": "x_api", "method": "official_username_check", "confidence": 0.99}},
        {"x": {"status": "taken", "source": "socialscan", "method": "registration_probe", "confidence": 0.9}},
    )
...
def test_legacy_not_found_never_becomes_verified_available():
    verdict = verdict_from_legacy_result(
        "instagram",
        "example",
        {"status": "not_found", "source": "public_web", "method": "public_profile", "confidence": 0.72, "occupancy": "not_found", "claimability": "unconfirmed"},
    )
    assert verdict["verdict"] == "likely_available"
    assert verdict["verdict"] != "available_verified"
...
def test_legacy_namecom_claimable_becomes_verified_available():
    evidence = legacy_result_to_evidence(
        "com",
        "example",
        {"status": "claimable", "source": "namecom_core_api", "method": "registrar_check_availability", "confidence": 0.99, "occupancy": "not_found", "claimability": "confirmed", "offer": {"provider": "name.com", "domain_name": "example.com"}},
    )
...
def test_attach_verification_verdicts_is_additive_per_platform():
    verdicts = attach_verification_verdicts(
        "example",
        {
            "com": {"status": "claimable", "source": "namecom_core_api", "method": "registrar_check_availability", "confidence": 0.99},
            "telegram": {"status": "taken", "source": "public_web", "method": "public_profile", "confidence": 0.85},
            "youtube": {"status": "not_found", "source": "youtube_data_api", "method": "official_handle_lookup", "confidence": 0.92},
        },
    )
