...
@media(max-width:860px){.shell{display:block}.controls{grid-template-columns:88px 1fr}.controls button{grid-column:1/-1}.history{position:fixed;z-index:5;left:12px;right:12px;bottom:12px;top:auto;height:min(55vh,520px);transform:translateY(calc(100% + 24px));transition:.2s}.history.open{transform:translateY(0)}.history-toggle{display:block!important;position:fixed;right:18px;bottom:18px;z-index:6;border-radius:999px}.grid{grid-template-columns:1fr}}.history-toggle{display:none}
...
function renderHistory(){const box=document.getElementById('historyList'),all=histories();box.innerHTML=all.length?all.map(x=>'<button class="history-item" onclick="loadHistory(\''+x.id+'\')"><b>'+esc(x.brief)+'</b><span>'+esc(x.created)+' · '+x.results.length+' names'+(x.done?' · complete':' · checking')+'</span></button>').join(''):'<div class="meta">No searches yet.</div>'}
