...
</style></head><body><main class="wrap"><div class="eyebrow">AI brand discovery · v4.1</div><h1>NameMachine</h1><p class="sub">Describe your project. AI generates names and checks .com plus six major platforms.</p><div class="controls"><select id="aiCount" aria-label="Number of AI names"><option>5</option><option selected>10</option><option>20</option></select><input id="brief" maxlength="500" placeholder="Describe your project"><button onclick="aiGo()">Generate + check 7</button></div><div id="grid" class="grid"></div><p class="warn">.com is checked via Verisign RDAP. Instagram/Telegram checks are best-effort and may return UNKNOWN because of anti-bot controls. Always do a final manual and trademark check.</p></main><script>
